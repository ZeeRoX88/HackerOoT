...
void Actor_SpawnOkamiFloorDustRing(PlayState* play, Actor* actor, Vec3f* posXZ, f32 radius, s32 amountMinusOne,
    ...
    for (i = amountMinusOne; i >= 0; i--) {
        ...
        accel.x = 2.5f * Math_SinF(angle);
        accel.z = 2.5f * Math_CosF(angle);

        s16 rotation; // 0xFFFF
        s16 diff = actor->world.rot.y - RAD_TO_BINANG(angle);
        if (diff < 0) {
            rotation = 1;
        } else {
            rotation = 0;
        ...
        EffectSsBomb_Spawn(play, &pos, &velocity, &accel, &effectPrimColor, 10, 40, 10, rotation, useLighting, 1);
